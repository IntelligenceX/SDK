...
from ix_identity import IdentityPortal
...
def main(argv=None):
    ...
    if 'INTELX_KEY' in os.environ:
        if args.identity:
            ix = IdentityPortal(os.environ['INTELX_KEY'])
        else:
            ix = intelx(os.environ['INTELX_KEY'])

    elif args.apikey:
        if args.identity:
            ix_identity = IdentityPortal(args.apikey)
        else:
    ...
    if args.identity:
        ...
        if args.exportaccounts:
            ...
            account = IdentityPortal.export_accounts(
                    ix,
                    args.identity,
